...
train = [
    dspy.Example(
        question=ds[i]["question"], answer=ds[i]["human_answers"][0], label="human"
    ).with_inputs("question", "answer")
    for i in range(5)
]
train += [
    dspy.Example(
        question=ds[i]["question"], answer=ds[i]["chatgpt_answers"][0], label="ai"
    ).with_inputs("question", "answer")
    for i in range(5)
]
...
lm = dspy.LM(
    f"openai/{model}", api_key=api_key, api_base=api_base, temperature=0, cache=False
)
